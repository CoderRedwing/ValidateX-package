const validate = require("../src/index").default;
...
describe("Credit Card Validation", () => {
  test("Valid credit card", () => {
    expect(validate.creditCard("4111111111111111").valid).toBe(true);
  });

  test("Invalid credit card (wrong length)", () => {
    expect(validate.creditCard("1234").valid).toBe(false);
  });

  test("Invalid credit card (non-numeric characters)", () => {
    expect(validate.creditCard("abcd1234efgh5678").valid).toBe(false);
  });
});
